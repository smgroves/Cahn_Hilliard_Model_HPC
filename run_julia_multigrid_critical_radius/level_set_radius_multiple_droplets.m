...
function [] = level_set_radius_multiple_droplets(R0,epsilon, indir, alpha, Nx,dt, total_time, suffix,contour_level)
    Ny = Nx;
    % total_time=.05;
    % dt_name="2.5e-5";
    ...
    CPC = 10;
    % epsilon=0.14;
    name=sprintf('phi_%d_%s_1.0e-6__R0_%s_eps_%s%s',Nx,string(timesteps),string(R0), string(epsilon), suffix)

   %name=sprintf('phi_%d_%s_1.0e-6__R0_%s_eps_%s%s',Nx,string(timesteps), string(R0) epsilon, suffix)
