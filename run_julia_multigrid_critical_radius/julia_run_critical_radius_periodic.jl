...
phi = initialization(nx, nx, method="droplet", h=1 / 128, R0 = R0, epsilon = gam)
...
time_passed = @elapsed multigrid_solver_with_periodic(phi, nx, tol, outdir, dt=dt, epsilon=gam, max_it=max_it, print_mass=false, print_e=false, overwrite=false, print_r=false, suffix="_R0_$(R0)_eps_$(round(gam, sigdigits = 5))_periodic", check_dir=false)
