...
function interface(x, beta, phi_1, phi_2)
    return (phi_2 + phi_1*np.exp(-x/beta))/(1+np.exp(-x/beta))
end

function initialization_from_function_v2(nx, ny, beta, phi_1, phi_2; R0=0.1, gam = 0.01)
    phi = zeros(Float64, nx, ny)
    h = 1/nx
    x = h .* (0:nx-1)
    y = h .* (0:ny-1)
    xx, yy = meshgrid(x, y)
    R = @.sqrt((xx - 0.5)^2 + (yy - 0.5)^2)
    phi = (phi_2 .+ phi_1.*@.exp((R.- R0)/(beta*gam)))./(1 .+ @.exp((R.-R0)/(beta*gam)))
    return phi
end 
