    ...
    for i in 1:nxt
        for j in 1:nyt
            if i > 1
                ...
                dadx_L = a[i, j] - a[nxt-1, j] #changed
            end
            if i < nxt
                dadx_R = (a[i+1, j] - a[i, j])
            else
                dadx_R = a[2, j] - a[i, j]  #changed
            end
            if j > 1
                dady_B = (a[i, j] - a[i, j-1])
            else
                dady_B = a[i, j] - a[i, nyt-1]  #changed
            end
            if j < nyt
                dady_T = (a[i, j+1] - a[i, j])
            else
                dady_T = (a[i, 2] - a[i, j]) #changed
            end
    ...
    for iter in 1:c_relax
        for i in 1:nxt
            for j in 1:nyt
                ...
                a[3] = -4 * Cahn / ht2 - 3 * (c_new[i, j])^2 #changed
                cnew_val = (c_new[i, j])
                d2f = -3 * (c_new[i, j])^2
...
function cahn(c_old, c_new, mu, nx, ny, dt, max_it_CH, tol, c_relax, xright, xleft, Cahn, n_level; suffix="", print_r=true)
    it_mg2 = 0
    resid2 = 1
...
function multigrid_solver_with_periodic(oc, nx, tol, outdir; max_it=1000, max_it_CH=10000, suffix="", overwrite=true, print_phi=true, print_mass=false, print_e=false, print_r=true, dt=2.5e-5, m=8, ns=10, epsilon=0.0, check_dir=true)
    while true
        ny = nx
